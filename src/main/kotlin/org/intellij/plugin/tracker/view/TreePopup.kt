...
class TreePopup(
        ...
        if (myScanResult.isValid(link)) {
            ApplicationManager.getApplication().runWriteAction {
                WriteCommandAction.runWriteCommandAction(myProject) {
                    ...
                    removeUpdatedLink(myScanResult.myLinkChanges, Pair(link, change), myProject)
                }
            }
        } else {
            showRefreshDialog()
     ...
     */
    private fun removeUpdatedLink(linkAndChange: MutableList<Pair<Link, Change>>, acceptedChange: Pair<Link, Change>, project: Project) {
        ...
        // Update view again to remove the updated link
