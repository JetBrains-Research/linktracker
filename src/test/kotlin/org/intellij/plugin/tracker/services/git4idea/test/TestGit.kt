...
class TestGitImpl : GitImpl() {
    ...
    override fun createEditor(
    ...
    ): GitInteractiveRebaseEditorHandler {
        ...
        val editor = object : GitInteractiveRebaseEditorHandler(project, root) {
            fun handleUnstructuredEditor(file: File): Boolean {
                ...
                return if (plainTextEditor != null) handleEditor(
                    file,
                    plainTextEditor
                ) else super.handleUnstructuredEditor(file.path)
            }

            fun handleInteractiveEditor(file: File): Boolean {
                val entriesEditor = interactiveRebaseEditor!!.entriesEditor
                return if (entriesEditor != null) handleEditor(file, entriesEditor) else super.handleInteractiveEditor(
                    file.path
                )
            }
    ...
    private fun fatalResult() =
        GitCommandResult(false, 128, listOf("fatal: error: $UNKNOWN_ERROR_TEXT"), emptyList<String>())
}
