...
fun GitPlatformTest.assertLogMessages(vararg messages: String) {
    val separator = "\u0001"
    val actualMessages =
        git("log -${messages.size} --pretty=${getPrettyFormatTagForFullCommitMessage(project)}$separator").split(
            separator
        )
...
fun ChangeListManager.assertOnlyDefaultChangelist() {
    HeavyPlatformTestCase.assertEquals(
        "Only default changelist is expected among: ${dumpChangeLists()}",
        1,
        changeListsNumber
    )
    HeavyPlatformTestCase.assertEquals(
        "Default changelist is not active",
...
fun ChangeListManager.assertChangeListExists(comment: String): LocalChangeList {
    ...
    HeavyPlatformTestCase.assertNotNull(
        "Didn't find changelist with comment '$comment' among: ${dumpChangeLists()}",
        list
    )
    return list!!
}
...
class ChangesBuilder {
    ...
    data class AChange(
        ...
        val gitDiffChangeMatcher: (GitChangeUtils.GitDiffChange) -> Boolean =
            { it -> matcher(it.status, it.beforePath, it.afterPath) }

        constructor(
            type: FileStatus,
            nameBefore: String?,
            nameAfter: String?,
            matcher: (FileStatus, FilePath?, FilePath?) -> Boolean
        ...
        override fun toString(): String {
            return when {
                type == FileStatus.ADDED -> "A: $nameAfter"
                type == FileStatus.DELETED -> "D: $nameAfter"
                nameBefore != nameAfter -> "M: $nameBefore -> $nameAfter"
                else -> "M: $nameAfter"
            }
        }
    }
